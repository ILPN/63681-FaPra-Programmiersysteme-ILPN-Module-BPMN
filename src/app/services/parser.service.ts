...
export class ParserService {
    ...
    constructor(
        ...
        this.layoutService.afterSugiyamaLayoutCallback =
    ...
    positionOfNodesAndEdgesChanged(
        nodes: BpmnNode[],
        ...
        for (const node of nodes) {
            if (this.text != []) {
                ...
                let matchLine = this.text.find((line) =>
                    line.startsWith(node.id)
                );
                if (matchLine != undefined) {
                    let index = this.text.indexOf(matchLine);

                    let matchLineNew = matchLine.replace(
                        /\(-?[0-9]*,-?[0-9]*\)/,
                        newCoordString
                    );

                    if (matchLine.match(/\(-?[0-9]*,-?[0-9]*\)/) === null) {
                        matchLineNew = matchLine
                            .replace(/[\n\r]/, '')
                            .concat(' ' + newCoordString);
                    }
    ...
    private parseTasks(line: string): BpmnNode | undefined {
        ...
        if (
            lineSplit[1] &&
            !lineSplit[1].startsWith('(') &&
            !(lineSplit[1] === description.split(' ').join(''))
        ) {
            ...
            switch (type.toLowerCase()) {
                ...
                default:
                    this.textareaError.emit(
                        'Ungültiger Task Typ bei ' + name + ': ' + type
                    );
            }
        }
    ...
    private parseEvents(line: string): BpmnNode | undefined {
        ...
        switch (type.toLowerCase()) {
            ...
            default:
                this.textareaError.emit(
                    'Ungültiger Event Typ bei ' + name + ': ' + type
                );
        }
    ...
    private parseGateways(line: string): BpmnNode | undefined {
        ...
        switch (type.toLowerCase()) {
            ...
            default:
                this.textareaError.emit(
                    'Ungültiger Gateway Typ bei ' + name + ': ' + type
                );
        }
                    ...
                    if (node2.id === lineSplit[1].trim()) {
                        ...
                        switch (type.toLowerCase()) {
                            case 'defaultflow':
                                ...
                                break;
                            case 'sequenceflow':
                                sequence = new BpmnEdge(name, node1, node2);
                                break;
                            case 'association':
                                sequence = new BpmnEdgeAssociation(
                                    name,
                                    node1,
                                    node2
                                );
