...
export class InputFieldComponent {
    ...
    fileUpload(file: File) {
        ...
        reader.addEventListener('load',() => {if(reader.result) {
    ...
    validateFormat(input: string): boolean {
        ...
        validateCategory (category: string, input: string):boolean {
            ...
            switch(category) {
                ...
                case "activities": regexp = /^[\w]+ [\w]+ "[\w]+"(?: \([0-9]*,[0-9]*\))?/; break;
                case "sequences": regexp = /^[\w]+ [\w]+ "[\w]+"(?: \([0-9]*,[0-9]*\))?/; break;
                case "events": regexp = /^[\w]+ [\w]+ "[\w]+"(?: \([0-9]*,[0-9]*\))?/; break;
                case "gateways": regexp = /^[\w]+ (and|or|xor)+(?: \([0-9]*,[0-9]*\))?/; break;
