...
export class DraggableEdge implements GetSvgManager {
    ...
    public get svgManager(): SvgManager {
        if (this._svgManager == undefined) {
            this._svgManager = new SvgManager('DraggableGraph', () =>
                this.svgCreation()
            );
        }
    ...
    getEndCornerDragHandle() {
        const dH = this.newDragHandle(
            this._edge.corners[this.edge.corners.length - 1]
        );
    ...
    addCircles(): SVGElement {
        ...
        for (let i = 1; i < this.edge.corners.length; i++) {
            ...
            if (i == this.edge.corners.length - 1)
                cornerPos = this.edge.nodeIntersection2;

            if (cornerBeforePos.distanceTo(cornerPos) > 10) {
    ...
    dragCircles(): SVGElement {
        ...
        for (const [i, corner] of this.edge.corners.entries()) {
            if (i == 0 || i == lastIndex) {
                const intersection =
                    i == 0
                        ? this.edge.nodeIntersection1
                        : this.edge.nodeIntersection2;
                const dragStartCir = Svg.circleNoStyleNoRadius(
                    intersection,
                    'dragHandleInnerCorner'
                );
                dragStartCir.onmousedown = (e) =>
                    this.dwg.startDrag(
                        e,
                        i == 0
                            ? this.getStartCornerDragHandle()
                            : this.getEndCornerDragHandle()
                    );
                const line = Svg.pathNoStyle(
                    [intersection, corner.getPos()],
                    'edgeLineInNode'
                );
                if (this.dragged) {
                    c.appendChild(line);
                    c.appendChild(
                        Svg.circleNoStyleNoRadius(corner.getPos(), 'endCircle')
                    );
                }
                c.appendChild(dragStartCir);
            } else {
                if (corner instanceof BpmnDummyEdgeCorner) {
                    const dragCir = Svg.circleNoStyleNoRadius(
                        corner.getPos(),
                        'dragHandleDummyCorner'
                    );
    ...
    addSnapsToDragHandle(dragHandle: DragHandle) {
        const index = this.edge.corners.findIndex(
            (c) => c == dragHandle.dragedElement
        );
        if (index == -1) return;
        if (index == 0) {
