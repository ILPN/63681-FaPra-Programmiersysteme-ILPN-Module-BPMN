...
export class Gateway extends Element {
    ...
    private _width: number = 50; // width
    private _height: number = this._width; // height
    ...
    public createSvg(): SVGElement {
        ...
        // G Gateway-Rechteck erstellen
        var g = this.createSvgElement('rect');
        g.setAttribute('width', `${this._width}`);
        g.setAttribute('height', `${this._height}`);
        g.setAttribute('stroke', 'rgb(0,0,0)');
        g.setAttribute('stroke-width', `${this._border}`);
        g.setAttribute('fill', 'white');
        g.setAttribute('transform', 'rotate(-45 ' + `${(this._width/2)}` + ' ' +`${this._height/2}`+')');
        this.addSVGtoColorChange(g);
        svg.append(g);
        svg.append(this.getTypeSvg());
        // G Gateway-Rechteck ende
    ...
    private getTypeOR(): SVGElement {
        var type_svg = this.createSvgElement('circle');
